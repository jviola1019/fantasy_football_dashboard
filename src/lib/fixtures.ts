...
const FIXTURE_ROWS: ReadonlyArray<{
   ...
   *
   *   - Teammates share a bye. Gibbs and LaPorta are both Detroit, both week 5
   *     -- and both are DIFFERENT positions, so that pair is not a clash.
   *   - Several teams share a week, exactly as the real schedule works. ATL,
   *     SF, PHI and DET are all week 5, which puts four running backs in one
   *     bye group -- so the stacking warning is reachable from the TOP of the
   *     board and survives into the eight-deep recommendation queue, instead of
   *     being dead code nobody ever sees fire.
   */
  byeWeek: number;
  ...
  { id: "jahmyr-gibbs",        name: "Jahmyr Gibbs",        position: "RB", team: "DET", perceivedValue: 83, trueValue: 90,  ownershipLeverage:  11, fragility: 31, trendingMomentum:  29, volatility: 46, opportunity: 86, confidence: 0.57, rosterSlot: "RB2", byeWeek: 5, pointsPerGame: 18.6, touchesPerGame: 18.3, sleeperId: "9221" },
  { id: "sam-laporta",         name: "Sam LaPorta",         position: "TE", team: "DET", perceivedValue: 68, trueValue: 77,  ownershipLeverage:  16, fragility: 24, trendingMomentum:  18, volatility: 29, opportunity: 74, confidence: 0.55, rosterSlot: "TE1", byeWeek: 5, pointsPerGame: 11.2, touchesPerGame: 6.8, sleeperId: "10859" },
  ...
  { id: "saquon-barkley",      name: "Saquon Barkley",      position: "RB", team: "PHI", perceivedValue: 94, trueValue: 92,  ownershipLeverage:  -6, fragility: 41, trendingMomentum:  24, volatility: 38, opportunity: 89, confidence: 0.59, rosterSlot: "RB4", byeWeek: 5,  pointsPerGame: 19.8, touchesPerGame: 21.6, sleeperId: "4866" },
