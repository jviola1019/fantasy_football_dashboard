...
- [ ] **External-adapter circuit breaker (optional, P3 — do not build speculatively).** `src/lib/http.ts` already does timeout + bounded exponential backoff + 429/`Retry-After` handling + Zod-validate→`unavailable` (never a fabricated fallback), and stale-data resilience is handled architecturally by the daily cron snapshots (serve last-good known data). A short-circuit breaker (open after N consecutive upstream failures, skip live calls during a cooldown) would only cap latency under a *sustained* upstream outage. **Not required to pass current CI and not a P0/P1** — the cron-snapshot model already bounds live-fetch exposure. Implement only if a real outage shows repeated per-request timeouts in production logs. Flagged during the 2026-06-08 Opus 4.8 hardening pass.
