...
- The lifecycle cron at `/api/cron/lifecycle-check` runs every 30 minutes and pulls each user's roster live via `src/lib/leagues/fetchLive.ts` (decrypts ESPN cookies / hits Sleeper, normalizes through `PlayerMarketRecordSchema`) before running the rules engine. FAAB ratio is not yet extracted from league settings, so the FAAB-depleted rule remains dormant until that field is parsed.
